...
class Solution:
    def validateIPv4(self, queryIP: str) -> bool:
        ...
        valid_char_set = set(list(string.digits) + ["."])
        if not all(map(lambda x: x in valid_char_set, queryIP)):
            return False
        ...
        def validate_part(x) -> bool:
            try:
                int_x = int(x)
                return all([0 <= int_x <= 255, str(int_x) == x])
            except Exception:
                return False

        return all(validate_part(x) for x in parts)

    def validateIPv6(self, queryIP: str) -> bool:
        # 2001:0db8:85a3:0000:0000:8a2e:0370:7334

        valid_char_set = set(list(string.digits) + list("abcdef") + [":"])
        if not all(map(lambda x: x in valid_char_set, queryIP)):
            return False
    ...
    def validIPAddress(self, queryIP: str) -> str:
        lower_query_ip = queryIP.lower()
        if self.validateIPv4(lower_query_ip):
            return "IPv4"
        elif self.validateIPv6(lower_query_ip):
            return "IPv6"
        else:
            return "Neither"
